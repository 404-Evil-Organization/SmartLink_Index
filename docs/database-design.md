...
| :--------------- | :------------------------------------- | :-------------------------------------- |
| id               | BIGINT PK                              | 企业唯一标识                            |
| user_id          | BIGINT UNIQUE                          | 关联`user.id`                           |
...
| status          | ENUM('draft','published','matched','closed') DEFAULT 'published' | 状态：草稿、已发布、已匹配、已关闭 |
...
| 字段名      | 类型/约束 | 说明                      |
| :---------- | :-------- | :------------------------ |
| demand_id   | BIGINT    | 关联`demand.id`，联合主键 |
| tag_id      | BIGINT    | 关联`tag.id`，联合主键    |
| create_time | DATETIME  | 记录创建时间              |
| update_time | DATETIME  | 最后更新时间              |
...
| category    | VARCHAR(50)          | 标签类别（如“服务类型”、“认证类型”） |
...
| 字段名      | 类型/约束 | 说明                                |
| :---------- | :-------- | :---------------------------------- |
| service_id  | BIGINT    | 关联`service_provider.id`，联合主键 |
| tag_id      | BIGINT    | 关联`tag.id`，联合主键              |
| create_time | DATETIME  | 记录创建时间                        |
| update_time | DATETIME  | 最后更新时间                        |
...
| :----------- | :--------------------- | :------------------------------------- |
| id           | BIGINT PK              | 评价唯一标识                           |
| coop_id      | BIGINT UNIQUE NOT NULL | 关联`cooperation.id`，一次合作一条评价 |
...
| year         | SMALLINT             | 年份                                      |
| quarter      | TINYINT              | 季度（1-4）或月份（1-12），根据统计粒度   |
| coop_density | DECIMAL(5,4)         | 合作密度（合作次数/企业总数）             |
...
- 频繁查询的字段：`user.username`（唯一索引）、`manufacture.region`、`service_provider.region`、`demand.status`、`demand.create_time`、`region_index.region`、`region_index.year/quarter`等，可酌情添加索引。
- 标签关联表（`demand_tag`、`service_tag`）的组合字段均作为联合主键，自动建立索引。
...
- **区域指数**：通过定时任务聚合`cooperation`、`manufacture`、`service_provider`数据，按区域和时间粒度计算并写入`region_index`。
