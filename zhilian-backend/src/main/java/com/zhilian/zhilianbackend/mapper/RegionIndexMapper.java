...
public interface RegionIndexMapper extends BaseMapper<RegionIndex> {
     ...
     */
    @Select("SELECT * FROM region_index WHERE year = #{year} " +
            "AND period_type = 'quarter' AND deleted = #{notDeletedTime}")
    ...
    @Select("SELECT DISTINCT year FROM region_index " +
            "WHERE period_type = 'quarter' AND deleted = #{notDeletedTime} " +
            "ORDER BY year DESC")
    List<Short> selectAvailableYears(@Param("notDeletedTime") LocalDateTime notDeletedTime);

    /**
     * 删除指定年份和季度的数据（用于重新计算）
     * 仅对当前未删除记录（deleted = notDeletedTime）进行逻辑删除，
     * 避免覆盖历史删除时间，确保 deleted 字段可作为审计时间使用。
     */
    @Update("UPDATE region_index SET deleted = NOW() WHERE year = #{year} " +
            "AND period_type = 'quarter' AND period_value = #{quarter} " +
            "AND deleted = #{notDeletedTime}")
    void deleteByYearAndQuarter(@Param("year") Short year, 
                                @Param("quarter") Byte quarter,
                                @Param("notDeletedTime") LocalDateTime notDeletedTime);
}