...
CREATE TABLE `tag` (
    ...
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
    UNIQUE INDEX uk_name_category_deleted (`name`, `category`, `deleted`),
    INDEX idx_deleted (`deleted`)
...
CREATE TABLE `demand_tag` (
    ...
    UNIQUE INDEX uk_demand_tag_deleted (`demand_id`, `tag_id`, `deleted`),
    INDEX idx_deleted (`deleted`)
...
CREATE TABLE `service_tag` (
    ...
    FOREIGN KEY (`tag_id`) REFERENCES `tag`(`id`) ON DELETE CASCADE,
    UNIQUE INDEX uk_service_tag_deleted (`service_id`, `tag_id`, `deleted`),
    INDEX idx_deleted (`deleted`)
...
CREATE TABLE `evaluation` (
    ...
    UNIQUE INDEX uk_coop_evaluator_role_deleted (`coop_id`, `evaluator_role`, `deleted`),
...
CREATE TABLE `region_index` (
    ...
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
    UNIQUE INDEX uk_region_year_period_deleted (`region`, `year`, `period_type`, `period_value`, `deleted`),
